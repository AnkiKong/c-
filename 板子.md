...
int man() {
    ...
    for (int i = 0; i < l; i++) {
        ...
        while (s2[i+ma[i]] == s2[i-ma[i]]) ma[i]++;
        if (i + ma[i] > mx) {
            mx = i + ma[i];
            id = i;
        }
...
void update(int pos, int val) {
    for (; pos<=len; pos+=lowbit(pos)) bit[pos]+=val;
}
int query(int pos) {
    int ans=0;
...
int main() {
    ...
    for (; t--; ) {
        ...
        len = man();
        n=strlen(s1);
        for (int i=0; i<=len; i++) wait[i].clear(), bit[i]=0;
        int k=1;
        for (int i=2; i<len; i+=2, k++) {
            p[k]=ma[i]/2-1;
            wait[k-p[k]].push_back(k);
        }
        ll ans=0;
        for (int i=1; i<=n; i++) {
            for (int a:wait[i])
...
struct bit {
    ...
    void add(int loc, int val) {
        for (; loc<=n; loc+=-loc&loc) cnt[loc]+=val;
    }
    int sum(int loc) {
        int ans=0;
        for (; loc; loc-=-loc&loc) ans+=cnt[loc];
        return ans;
    }
}bt;
...
void solve(int l, int r) {
    ...
    int i=l+1, j=mid+1;
    for (; i<=mid; i++) {
        for (; da[i].val>da[j].val && j<=r; j++) {
            bt.add(da[j].del, 1);
        }
        da[i].ans+=bt.sum(m+1)-bt.sum(da[i].del);
    }
    i=l+1, j=mid+1;
    for (; i<=mid; i++) {
        for (; da[i].val>da[j].val && j<=r; j++) {
    ...
    i=mid, j=r;
    for (; j>mid; j--) {
        for (; da[i].val>da[j].val && i>l; i--) {
            bt.add(da[i].del, 1);
        }
        da[j].ans+=bt.sum(m+1)-bt.sum(da[j].del);
    }
    i=mid, j=r;
    for (; j>mid; j--) {
        for (; da[i].val>da[j].val && i>l; i--) {
...
int main()
{
    ...
    for (int i=1; i<=n; i++) {
        if (!da[i].del) da[i].del=m+1;
    }
    ll res=0;
    for (int i=1; i<=n; i++) {
        res+=bt.sum(n)-bt.sum(da[i].val);
        bt.add(da[i].val, 1);
    }
    for (int i=1; i<=n; i++) {
        bt.add(da[i].val, -1);
...
``` c++
// 差分约束(求最大值：转为a - b <= c,add(a, b, c)跑最短路; 求最小值：转为a - b >= c, add(b, a, c)跑最远路)
...
int main() {
    ...
    for (int i = 0, x, a, b; i < k; i++) {
        ...
        switch (x) {
            ...
            case 2: add(a, b, 1); if(a == b) {printf("-1"); return 0;}break;//a<b
            case 3: add(b, a, 0); break;//a>=b
            case 4: add(b, a, 1); if(a == b) {printf("-1"); return 0;}break;//a>b
...
- $(I*\mu)()=(id_1)()$
- $d(i\cdot j)=\sum_{x|i}\sum_{y|j}[gcd(x,y)=1]=\sum_{p|i,p|j}\mu(p)d\left(\frac{i}{p}\right)d\left(\frac{j}{p}\right)$
- ${\sum_{m|n}φ(m)}=n$  
- $\sum^n_i\sum^m_j [gcd(i, j)==k] = \sum^n_i\mu(d)\lfloor\frac{n}{kd}\rfloor\lfloor\frac{m}{kd}\rfloor$
- $φ^2(n)=φ(n^2)$  
